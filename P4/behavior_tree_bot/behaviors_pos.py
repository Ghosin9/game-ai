...
def attack_optimal_neutral(state):
    my_planets = sorted(state.my_planets(), key=lambda p: p.num_ships)
    ...
    for my_planet in state.my_planets():
        ...
        if len(state.enemy_planets()) != 0: #no fleet en route from here
            ...
            try:
                ...
                for fleet in state.enemy_fleets():
                    if fleet.destination_planet == best_prospect:
...
def spread_to_optimal(state):
    order_issued = False
    for my_planet in state.my_planets():
        return True
        close_planets = closest_planets(state, my_planet, 5)

        prospect = None
        for close_planet in close_planets:
            if my_fleet_en_route(state, close_planet):
                pass
            else:
                if prospect == None:
                    prospect = close_planet
                    pass
                if prospect.growth_rate / prospect.num_ships < close_planet.growth_rate / close_planet.num_ships:
                    prospect = close_planet
        
        if prospect in state.neutral_planets:
            if neutral_planet_cost == -1:
                pass
            if issue_order(state, my_planet, prospect, neutral_planet_cost):
                order_issued = True
        else:
            if issue_order(state, my_planet, prospect, enemy_planet_cost):
                order_issued = True
    return order_issued

#helper function to return num_planets closest planets not owned by player to planet
def closest_planets(state, planet, num_planets):
    #setup all planets - the given planet in a list
    distance_sorted_planets = state.enemy_planets.copy() + state.neutral_planets.copy()
...
def enemy_planet_cost(state, planet, source):
    e_fleet = enemy_fleet_en_rout(state, planet)
    if e_fleet !- None:
        return e_fleet.num_ships + planet.num_ships + planet.growth_rate * distance(source.ID, planet.ID) + 1
    return + planet.num_ships + planet.growth_rate * distance(source.ID, planet.ID) + 1